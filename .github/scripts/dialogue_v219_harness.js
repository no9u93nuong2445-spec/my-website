...
test("语义识别同时给出动作对象和上下文", () => {
  ...
  const actions = analysis.semantic?.actions || [];
  return actions.includes("回应情绪") && actions.includes("自我分享") && actions.includes("提问了解") && analysis.semantic.topics.includes("food") && analysis.semantic.object !== "当前交流" && /承接|回应/.test(analysis.semantic.context);
});
